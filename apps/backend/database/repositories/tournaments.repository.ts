...
export class TournamentsRepository {
	...
	async findByCreator(creatorId: number) {
		return await db
			.select()
			.from(tournaments)
			.where(eq(tournaments.creator, creatorId));
	}
	...
	async getUserTournaments(userId: number) {
		...
		const tournamentsWithCounts = await Promise.all(
			result.map(async (tournament) => {
				...
				const pokemonTeam = await this.getPlayerTeamPokemon(
					tournament.id,
					userId,
				);
	...
	async getAvailableTournaments(userId: number) {
		const result = await db
			...
			.from(tournaments)
			.where(
				and(eq(tournaments.isStarted, false), eq(tournaments.isEnded, false)),
			);
		...
		const tournamentsWithStatus = await Promise.all(
			result.map(async (tournament) => {
				const currentPlayers = await this.getPlayerCount(tournament.id);
				const stages = await this.getTournamentStages(tournament.id);
				const isParticipating = await this.isPlayerInTournament(
					tournament.id,
					userId,
				);
	...
	async isPlayerInTournament(tournamentId: number, userId: number) {
		const [player] = await db
			...
			.where(
				and(
					eq(tourPlayers.tournamentId, tournamentId),
					eq(tourPlayers.userId, userId),
				),
			);
		return !!player;
	}
	...
	async updateStageTeam(
		...
		return await db.transaction(async (tx) => {
			await tx
				.delete(teamPokemon)
				.where(
					and(eq(teamPokemon.teamId, teamId), eq(teamPokemon.stageId, stageId)),
				);
