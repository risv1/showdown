...
export class MatchesRepository {
  private async parseReplayData(replayUrl: string): Promise<ParsedMatchData> {
    try {
      const jsonUrl = replayUrl.endsWith(".json") ? replayUrl : `${replayUrl}.json`;
  ...
  private async getUserByUsername(username: string): Promise<{ id: number } | null> {
    const [user] = await db
      .select({ id: users.id })
      .from(users)
  ...
  async findById(id: number): Promise<Match | null> {
    const [match] = await db.select().from(tourMatches).where(eq(tourMatches.id, id)).limit(1);
    return match || null;
  }
  ...
  async getPlayerStats(
    ...
    const playerMatches = await db
      .select()
      .from(tourMatches)
      .where(and(eq(tourMatches.tournamentId, tournamentId), eq(tourMatches.winnerId, playerId)));

    const losses = await db
      .select()
      .from(tourMatches)
      .where(and(eq(tourMatches.tournamentId, tournamentId), eq(tourMatches.loserId, playerId)));

    const points = playerMatches.reduce((acc, match) => acc + (match.pointsWon || 0), 0);
  ...
  async updateMatch(id: number, updates: Partial<CreateMatchData>): Promise<Match | null> {
    const [match] = await db
      .update(tourMatches)
      .set({
        ...updates,
