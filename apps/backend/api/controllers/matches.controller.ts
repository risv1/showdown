...
export class MatchesController {
	async createMatch(c: Context) {
		try {
			...
			if (tournament.creator !== Number.parseInt(userId)) {
				return c.json(
					{ error: "Only tournament creator can add matches" },
					403,
				);
			}
	...
	async updateMatch(c: Context) {
		try {
			...
			const tournament = await tournamentsRepository.findById(
				match.tournamentId,
			);
			if (!tournament) {
				return c.json({ error: "Tournament not found" }, 404);
			...
			const isParticipant =
				match.player1Id === Number.parseInt(userId) ||
				match.player2Id === Number.parseInt(userId);
