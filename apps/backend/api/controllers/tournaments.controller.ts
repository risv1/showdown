...
export class TournamentsController {
		...
		try {
			...
			if (stages[0].playersSelected > tournamentData.maxPlayers) {
				return c.json(
					{
						error:
							"First stage cannot select more players than tournament maximum",
					},
					400,
				);
			}
			...
			for (let i = 1; i < stages.length; i++) {
				if (stages[i].playersSelected > stages[i - 1].playersSelected) {
					return c.json(
						{
							error:
								"Each stage must select fewer or equal players than the previous stage",
						},
						400,
					);
				}
	...
	async joinTournament(c: Context) {
		try {
			...
			if (tournament.isStarted) {
				return c.json(
					{ error: "Cannot join tournament that has already started" },
					400,
				);
			}
			...
			if (tournament.joinsDisabled) {
				return c.json(
					{ error: "Joins are currently disabled for this tournament" },
					400,
				);
			}
			...
			const currentPlayerCount =
				await tournamentsRepository.getPlayerCount(tournamentId);
			if (currentPlayerCount >= tournament.maxPlayers) {
				return c.json({ error: "Tournament is full" }, 400);
			}

			await tournamentsRepository.addPlayerToTournament(
				tournamentId,
				Number.parseInt(userId),
			);

			await tournamentsRepository.createTeamForPlayer(
				tournamentId,
				Number.parseInt(userId),
			);
	...
	async submitStageTeam(c: Context) {
		try {
			...
			const team = await tournamentsRepository.getPlayerTeam(
				tournamentId,
				Number.parseInt(userId),
			);
			if (!team) {
	...
	async getTournament(c: Context) {
		try {
			...
			const playerCount =
				await tournamentsRepository.getPlayerCount(tournamentId);
			const stages =
				await tournamentsRepository.getTournamentStages(tournamentId);
	...
	async getUserTournaments(c: Context) {
		try {
			...
			const tournaments = await tournamentsRepository.getUserTournaments(
				Number.parseInt(userId),
			);
	...
	async getTournamentDetails(c: Context) {
		try {
			...
			if (!isParticipating && tournament.creator !== Number.parseInt(userId)) {
				return c.json(
					{ error: "Not authorized to view tournament details" },
					403,
				);
			}
			...
			const stages =
				await tournamentsRepository.getTournamentStages(tournamentId);
			const matches =
				await matchesRepository.getTournamentMatches(tournamentId);
			...
			const players =
				await tournamentsRepository.getTournamentPlayers(tournamentId);
			const playersWithStats = await Promise.all(
				players.map(async (player) => {
					const stats = await matchesRepository.getPlayerStats(
						tournamentId,
						player.userId,
					);
	...
	async disableJoins(c: Context) {
		try {
			...
			if (tournament.creator !== Number.parseInt(userId)) {
				return c.json(
					{ error: "Only tournament creator can disable joins" },
					403,
				);
			}
	...
	async updateTournamentStages(c: Context) {
		try {
			...
			const body = await c.req.json();
			const { stages } = z
				.object({ stages: z.array(stageSchema).min(1) })
				.parse(body);
			...
			if (tournament.creator !== Number.parseInt(userId)) {
				return c.json(
					{ error: "Only tournament creator can update stages" },
					403,
				);
			}
			...
			if (tournament.isStarted) {
				return c.json(
					{ error: "Cannot update stages after tournament has started" },
					400,
				);
			}
	...
	async endTournament(c: Context) {
		try {
			...
			if (tournament.creator !== Number.parseInt(userId)) {
				return c.json(
					{ error: "Only tournament creator can end the tournament" },
					403,
				);
			}
			...
			if (!tournament.isStarted) {
				return c.json(
					{ error: "Cannot end tournament that hasn't started" },
					400,
				);
			}
