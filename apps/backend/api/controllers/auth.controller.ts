...
import {
	convertShowdownTimestamp,
	fetchShowdownUserData,
} from "../../utils/showdown.utils";
...
export class AuthController {
	async register(c: Context) {
		try {
			...
			const existingUserByUsername = await usersRepository.findByUsername(
	...
	async login(c: Context) {
		try {
			...
			if (!user.showdownJoinDate) {
				const showdownData = await fetchShowdownUserData(
					validatedData.username,
				);
				if (showdownData && showdownData.registertime) {
					const showdownJoinDate = convertShowdownTimestamp(
						showdownData.registertime,
					);
					await usersRepository.updateShowdownJoinDate(
						user.id,
						showdownJoinDate,
					);
	...
	async refreshShowdownData(c: Context) {
		try {
			...
			if (!showdownData) {
				return c.json(
					{ error: "Unable to fetch Showdown data for this username" },
					404,
				);
			}
			...
			if (showdownData.registertime) {
				...
				const result = await usersRepository.updateShowdownJoinDate(
					user.id,
					showdownJoinDate,
				);
				if (result) {
